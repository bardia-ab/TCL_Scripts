proc gen_tiles {wires {filter 1}} {
	set tiles [list]
	foreach wire $wires {
		set wire_ports [split $wire \t]
		foreach port $wire_ports {
			lappend tiles [lindex [split $port /] 0]
		}
	}
	set tiles [lsort -unique $tiles]
	if {$filter == 1} {
		foreach tile $tiles {
			if {[string match INT_X* $tile] == 0} {
				set index [lsearch $tiles $tile]
...
proc gen_clb {tile} {
	...
	print_list [lsort $lst1] $tile1 "C:/Users/Sony/Desktop/graph/clb"
	if {[string match None $tile2] == 0} {
		print_list [lsort $lst2] $tile2 "C:/Users/Sony/Desktop/graph/clb"
	}
}

proc get_graph {} {
	set tiles [get_tiles -filter {NAME =~ INT_X*}]
	foreach tile $tiles {
		print_list [gen_pips $tile] "PIPs.$tile" "C:/Users/Sony/Desktop/graph/pips"
		print_list [gen_wires $tile] "WIREs.$tile" "C:/Users/Sony/Desktop/graph/wires"
		gen_clb $tile
	}
}

proc get_graph {tile} {
	set wires [gen_wires $tile]
	set tiles [gen_tiles $wires]
	foreach tile1 $tiles {
		print_list [gen_pips $tile1] "PIPs.$tile1" "C:/Users/Sony/Desktop/graph/pips"
		print_list [gen_wires $tile1] "WIREs.$tile1" "C:/Users/Sony/Desktop/graph/wires"
		gen_clb $tile1
	}
}